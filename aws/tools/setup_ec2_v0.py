...
cfg = Cfg({
    "name" : "klein",      # Unique name for this specific configuration
    "key_name": "klein",          # Necessary to ssh into created instances
    # Cluster topology
    ...
    # "image_id": "ami-0436a690e91c0b7f1",
    "image_id": "ami-0a7634308c3d15a17",
    # Launch specifications
    "spot_price" : "0.27",                 # Has to be a string
    # SSH configuration
    "ssh_username" : "ubuntu",            # For sshing. E.G: ssh ssh_username@hostname
    "path_to_keyfile" : "/mnt/nfs/scratch1/dthai/Projects/codednet/aws/klein.pem",

    # NFS configuration
    # To set up these values, go to Services > ElasticFileSystem > Create new filesystem, and follow the directions.
    #"nfs_ip_address" : "172.31.3.173",         # us-west-2c
    #"nfs_ip_address" : "3.138.236.125",          # us-west-2a
    "nfs_ip_address" : "172.31.20.93",          # us-east-1a
...
def mxnet_ec2_run(argv, configuration):
    ...
    def launch_instances():
       ...
       for (instance_type, count) in specs:
          launch_specs = {"KeyName" : configuration["key_name"],
                          ...
                          "SecurityGroups": ["launch-wizard-10"]}#["default"]}
          if method == "spot":
    ...
    def run_mxnet_grid_search(argv, port=1334):
        ...
        worker_host_string = ",".join([x.public_ip_address+":"+str(port) for x in machine_assignments["master"] + machine_assignments["worker"]])
    ...
    def get_hosts(argv, port=22):
        ...
        for instance in machine_assignments["master"] + machine_assignments["worker"]:
            count += 1
            print('{}\tdeeplearning-worker{}'.format(instance.public_ip_address, count), end='\n', file=hosts_out)
        hosts_out.flush()
        hosts_out.close()
        ...
        for instance in machine_assignments["master"] + machine_assignments["worker"]:
            count += 1
            print('{}'.format(instance.public_ip_address), end='\n', file=hosts_alias_out)
        hosts_alias_out.flush()
        hosts_alias_out.close()
